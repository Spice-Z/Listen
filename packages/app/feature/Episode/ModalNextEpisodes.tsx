...
function ModalNextEpisodes() {
  ...
  const playPauseButton = useMemo(() => {
    ...
    return isPlaying ? (
      <PauseIcon fill={theme.color.textMain} width={20} height={20} />
    ) : (
      <PlayIcon fill={theme.color.textMain} width={20} height={20} />
    );
  }, [isLoading, isPlaying]);
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View style={styles.episodeContainer}>
              <EpisodeList id={id} title={title} imageUrl={artwork} onPress={() => {}} />
            </View>
          );
        }}
