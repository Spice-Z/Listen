...
export const generateTranscript = functions
  ...
  .https.onCall(async (data, _) => {
    ...
    for (const episode of episodes) {
      ...
      const episodePendingTranscriptSnapshot = await admin
        .firestore()
        .collection(TRANSCRIPT_PENDING_EPISODES_DOCUMENT_NAME)
        .where('episodeId', '==', episode.id)
        .get();

      if (!episodePendingTranscriptSnapshot.empty) {
        const episodePendingTranscriptDoc = episodePendingTranscriptSnapshot.docs[0];
        await episodePendingTranscriptDoc.ref.delete();
      }
    }
